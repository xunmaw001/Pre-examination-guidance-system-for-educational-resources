<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="网盘链接" prop="wangpanlianjie">
          <el-input v-model="ruleForm.wangpanlianjie" 
              placeholder="网盘链接" clearable  :readonly="ro.wangpanlianjie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="网盘链接" prop="wangpanlianjie">
              <el-input v-model="ruleForm.wangpanlianjie" 
                placeholder="网盘链接" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.wentimiaoshu = true;
          ...
          if(o=='wangpanlianjie'){
            this.ruleForm.wangpanlianjie = obj[o];
	    this.ro.wangpanlianjie = true;
            continue;
          }
          if(o=='fabushijian'){
	...
	this.ruleForm.wentimiaoshu = this.ruleForm.wentimiaoshu.replace(reg,'../../../ssmjobfi/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wentixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wentixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
