  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='jiaoshi'"  label="讲解价格" prop="jiangjiejiage">
          <el-input v-model="ruleForm.jiangjiejiage"               placeholder="讲解价格" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="flag=='jiaoshi'"  label="学院" prop="xueyuan">
          <el-select v-model="ruleForm.xueyuan"  placeholder="请选择学院">
            <el-option
                v-for="(item,index) in jiaoshixueyuanOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  mounted() {
    ...
    this.$http({
      ...
      if (data && data.code === 0) {
        this.jiaoshixueyuanOptions = data.data;
      } else {
        this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    onUpdateHandler() {
      ...
      if( 'jiaoshi' ==this.flag && this.ruleForm.jiangjiejiage&&(!isNumber(this.ruleForm.jiangjiejiage))){
